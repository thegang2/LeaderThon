...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        checkplugins(path)
        name = "iqLeaderThon.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"iqLeaderThon/plugins/{shortname}.py")
            name = f"iqLeaderThon.plugins.{shortname}"
        else:
        ...
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.bot = iqLeaderThon
        mod.LOGS = LOGS
        mod.Config = Config
        mod._format = _format
        mod.tgbot = iqLeaderThon.tgbot
        ...
        mod.borg = iqLeaderThon
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["iqLeaderThon.plugins." + shortname] = mod
        LOGS.info(f"[تم تحميل : {shortname}] \n ———————× IQ")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    iqLeaderThon.remove_event_handler(i)
    ...
    try:
        name = f"iqLeaderThon.plugins.{shortname}"
        for i in reversed(range(len(iqLeaderThon._event_builders))):
            ev, cb = iqLeaderThon._event_builders[i]
            if cb.__module__ == name:
                del iqLeaderThon._event_builders[i]
    except BaseException:
        raise ValueError
