...
from Arab import iqLeaderThon, catversion, StartTime
...
CATLOGO = "https://telegra.ph/file/46cc087ec231116cf18d1.jpg"
...
@iqLeaderThon.tgbot.on(InlineQuery)
async def inline_handler(event):  
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**iqLeaderThonbot"):
            buttons = [
                (
            ...
            builder = event.builder
            secret = os.path.join("./iqLeaderThon", "secrets.txt")
            try:
        ...
        await event.answer([result] if result else None)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
