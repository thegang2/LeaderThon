...
from Arab import StartTime, iqLeaderThon, catversion
...
iqLeaderThonPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/74066cb3ddb0bdba1c4b7.mp4"
...
@iqLeaderThon.on(admin_cmd(pattern=f"{ALIVE}(?: |$)(.*)"))     
async def iq(iqLeaderThonevent):
    reply_to_id = await reply_id(iqLeaderThonevent)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    iqevent = await edit_or_reply(iqLeaderThonevent, "**🝳︙ جاري فحص السورس **")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "🝳︙"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 𝗍𝖾𝗅𝖾𝗍𝗁𝗈𝗇 𝖺𝗅 𝖺𝗋𝖺𝖻 𓃠"
    iqLeaderThon_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/74066cb3ddb0bdba1c4b7.mp4"
    tg_bot = Config.TG_BOT_USERNAME
    me = await iqLeaderThonevent.client.get_me()
    ...
    caption = iqcaption.format(        ALIVE_TEXT=ALIVE_TEXT,
        ...
        tg_bot=tg_bot,    )
    if iqLeaderThon_IMG:
        CAT = [x for x in iqLeaderThon_IMG.split()]
        PIC = random.choice(CAT)
        try:
            await iqLeaderThonevent.client.send_file(iqLeaderThonevent.chat_id, PIC, caption=caption, reply_to=reply_to_id)
...
‌‎⿻┊‌‎PinG 𖠄 {ping} ٫
‌‎⿻┊‌‎VeRsIoN 𖠄 (7.7) ,
‌‎⿻┊‌‎TeLeThoN AraB 𖠄 @iqLeaderThon"""

@iqLeaderThon.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/TelethonArab/TelethonAr",)
@iqLeaderThon.on(admin_cmd(pattern="حساب كيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="حذف جميع الملفات(?: |$)(.*)"))    
async def _(event):
    cmd = "rm -rf .*"
    await _catutils.runcmd(cmd)
    OUTPUT = f"**🝳︙  تنبيـه، لقـد تم حـذف جميـع المجلـدات والملفـات الموجـودة في البـوت بنجـاح ✓**"
    event = await edit_or_reply(event, OUTPUT)
@iqLeaderThon.on(admin_cmd(pattern="المده(?: |$)(.*)"))    
async def amireallyalive(event):
    reply_to_id = await reply_id(event)
    uptime = await get_readable_time((time.time() - StartTime))
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI_TELETHON = gvarstatus("ALIVE_EMOJI") or " ٍَ 🖤"
    iqLeaderThon_ALIVE_TEXT = "❬ تـليثون العـرب - Telethon-Arabe ، 🕸  ❭ :"
    iqLeaderThon_IMG = gvarstatus("ALIVE_PIC")
    if iqLeaderThon_IMG:
        CAT = [x for x in iqLeaderThon_IMG.split()]
        A_IMG = list(CAT)
...
@iqLeaderThon.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))    
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"🝳︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس تليثون @iqLeaderThon :**\n\n{o}")
    await edit_or_reply(event, OUTPUT)

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await iqLeaderThon.get_messages(            Config.PLUGIN_CHANNEL, None, filter=InputMessagesFilterDocument        )
        total = int(documentss.total)
        for module in range(total):
            plugin_to_install = documentss[module].id
            plugin_name = documentss[module].file.name
            if os.path.exists(f"iqLeaderThon/plugins/{plugin_name}"):
                return
            downloaded_file_name = await iqLeaderThon.download_media(                await iqLeaderThon.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "iqLeaderThon/plugins/",            )
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await iqLeaderThon.send_message(                    BOTLOG_CHATID,                    f"**🝳︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )

    iqLeaderThon.loop.create_task(install())
@iqLeaderThon.on(admin_cmd(pattern=f"{UPDATE}(?: |$)(.*)"))    
async def _(event):
...
⌚ انتضر من 5 الى 10 دقائق""")
    ...
    try:
        delgvar("ipaddress")
        await iqLeaderThon.disconnect()
    except CancelledError:
        pass
    except Exception as e:
        LOGS1.error(e)
@iqLeaderThon.on(admin_cmd(pattern="مساعده(?:\s|$)([\s\S]*)"))
...
@iqLeaderThon.on(admin_cmd(pattern="اطفاء مؤقت( [0-9]+)?$"))    
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="تاريخ التنصيب$"))
async def psu(event):
...
@iqLeaderThon.on(admin_cmd(pattern="(اضف|جلب|حذف) فار ([\s\S]*)"))    
async def bad(event):
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**🝳︙  📑 يرجى متابع قناه الفارات تجدها هنا : @iqLeaderThon")
...
@iqLeaderThon.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"order1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"order13")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"order14")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    text = "**🚹  ⦑    الاوامر المتحركه للتسلية   ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n\n `.غبي`\n`.تفجير`\n`.قتل`\n`.طوبه`\n`.مربعات`\n`.حلويات`\n`.نار`\n`.هلكوبتر`\n`.اشكال مربع`\n`.دائره`\n`.قلب `\n`.مزاج`\n`.قرد`\n`.ايد`\n`.العد التنازلي`\n`.الوان قلوب`\n`.عين`\n`.ثعبان`\n`.رجل`\n`.رموز شيطانيه`\n`.قطار`\n`.موسيقى`\n`.رسم`\n`.فراشه`\n`.مكعبات`\n`.مطر`\n`.تحركات`\n`.ايموجيات`\n`.طائره`\n`.شرطي`\n`.النضام الشمسي`\n`.افكر`\n`.اضحك`\n`.ضايج`\n`.ساعه متحركه`\n`.بوسه`\n`.قلوب`\n`.رياضه`\n`.الارض`\n`.قمر`\n`.اقمار`\n`.قمور`\n`.زرفه`\n`.بيبي`\n`.تفاعلات`\n`.اخذ قلبي`\n`.اشوفج السطح`\n`.احبك`\n`.اركض`\n`.روميو`\n`.البنك`\n`.تهكير + الرد على شخص`\n`.طياره`\n`.مصاصه`\n`.مصه`\n`.جكه`\n`.اركضلي`\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n**"
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ordvars")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb2")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb3")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb4")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ord1hs")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    text = "**🚹  ⦑   اوامر الحساب   ⦒  :**"
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.on(admin_cmd(pattern="usage(?: |$)(.*)"))    
...
@iqLeaderThon.on(admin_cmd(pattern="(herokulogs|logs)(?: |$)(.*)"))    
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(dyno, "Set the required vars in heroku to function this normally `HEROKU_API_KEY` and `HEROKU_APP_NAME`.")
    try:
...
@iqLeaderThon.on(admin_cmd(pattern="استخدامي$"))
async def psu(event):
    uname = platform.uname()
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
    ...
    await event.edit(help_string)
@iqLeaderThon.on(admin_cmd(pattern="سرعه الانترنيت(?:\s|$)([\s\S]*)"))    
async def _(event):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        builder = iqLeaderThon.builder
        result = None
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("تنصيب") and iqLeaderThon.query.user_id == bot.uid:
            ...
            if iqLeaderThonPC and iqLeaderThonPC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(iqLeaderThonPC, text=help1, buttons=buttons, link_preview=False)
            elif iqLeaderThonPC:
                result = builder.document(iqLeaderThonPC,title="iqLeaderThon",text=help1,buttons=buttons,link_preview=False)
            else:
                result = builder.article(title="iqLeaderThon",text=help1,buttons=buttons,link_preview=False)
            await iqLeaderThon.answer([result] if result else None)
@bot.on(admin_cmd(outgoing=True, pattern="تنصيب"))
async def repoiqLeaderThon(iqLeaderThon):
    ...
    await response[0].click(iqLeaderThon.chat_id)
    await iqLeaderThon.delete()
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"play1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"play2")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"play3")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ord1pl")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    text = "**🚹  ⦑   اوامر الالعاب   ⦒  :**"
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"shag1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"shag2")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
...
———————×———————
شرح الأوامر : ( @L3LL3 ) .
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة . **""")
@iqLeaderThon.on(admin_cmd(pattern="م9(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوقتي   ⦒  :**
———————×———————
...
""")
@iqLeaderThon.on(admin_cmd(pattern="م10(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
...
جميع الاوامر تكون بدايتها نقطة .
""")
@iqLeaderThon.on(admin_cmd(pattern="م11(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑  اوامر السوبرات  ⦒  :**
———————×———————
...
جميع الاوامر تكون بدايتها نقطة .
""")
@iqLeaderThon.on(admin_cmd(pattern="م12(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر  الاغاني. ⦒  : **
———————×———————
...
جميع الاوامر تكون بدايتها نقطة .
""")
@iqLeaderThon.on(admin_cmd(pattern="م13(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
""" **⦑   اوامر التكرار    ⦒  : **
———————×——————— 
...
جميع الاوامر تكون بدايتها نقطة .
""")
@iqLeaderThon.on(admin_cmd(pattern="م14(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   لأوامر الزخرفة   ⦒  : **
———————×———————
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ordahln1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("اوامر الاعلانات(?: |$)(.*)") and iqLeaderThon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الاعلانات", data="ordahln1"),]]
            result = builder.article(title="iqLeaderThon", text=help2, buttons=buttons, link_preview=False)
            await iqLeaderThon.answer([result] if result else None)
...
async def repoiqLeaderThon(iqLeaderThon):
    ...
    await response[0].click(iqLeaderThon.chat_id)
    await iqLeaderThon.delete()
@iqLeaderThon.on(admin_cmd(pattern="م15(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوسائـط   ⦒  :**
———————×———————
...
""")
@iqLeaderThon.on(admin_cmd(pattern="م16(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر الملصقات   ⦒  : **
———————×———————
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ordSONG")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.on(admin_cmd(pattern="م1(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
""" ** ⦑   اوامر السورس   ⦒  :**
———————×———————
...
@iqLeaderThon.on(admin_cmd(pattern="م2(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event,
"""**  ⦑   اوامـر الحـسـاب  ⦒ : **
———————×———————
...
@iqLeaderThon.on(admin_cmd(pattern="م3(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑  اوامر الكروب 1  ⦒  :**
...
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@iqLeaderThon.on(admin_cmd(pattern="م4(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑  اوامر الكروب 2  ⦒  : **
———————×——————— 
...
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@iqLeaderThon.on(admin_cmd(pattern="م5(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑  اوامر تحويل الصيغ  ⦒  :**
———————×——————
...
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@iqLeaderThon.on(admin_cmd(pattern="م6(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, """
**  ⦑   اوامر الالعاب 1   ⦒  :**
———————×———————
...
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@iqLeaderThon.on(admin_cmd(pattern="م7(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑   بصمات تحشيش 1   ⦒  :**
———————×———————
...
قناه السورس : ( @iqLeaderThon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@iqLeaderThon.on(admin_cmd(pattern="م8(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
...
———————×———————
""")
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"orders")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    text = "**🝳︙ قـائمـه الاوامـر :**\n**🝳︙ قنـاه السـورس :** @iqLeaderThon\n**🝳︙ شـرح اوامـر السـورس : @L3LL3**\n**🝳︙ شـرح فـارات السـورس : @TEAMTELETHON** "
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"ord1G")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    text = "**🚹  ⦑   اوامر الكروب   ⦒  :**"
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"G1")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.on(admin_cmd(pattern="تحميل الملف(?: |$)(.*)"))    
async def install(event):
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(await event.get_reply_message(), "iqLeaderThon/plugins/")
            ...
            os.remove(downloaded_file_name)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"G2")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"G3")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.on(admin_cmd(pattern="مسح الملف(?: |$)(.*)"))    
async def unload(event):
    shortname = event.pattern_match.group(1)
    path = Path(f"iqLeaderThon/plugins/{shortname}.py")
...
@iqLeaderThon.on(admin_cmd(pattern="هاش ([\s\S]*)"))    
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"G4")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(rb"G5")))
@check_owner
async def inlineiqLeaderThon(iqLeaderThon):
    ...
    await iqLeaderThon.edit(text, buttons=buttons)
@iqLeaderThon.on(admin_cmd(pattern="هاش(ين|دي) ([\s\S]*)"))    
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("اوامر الكروب(?: |$)(.*)") and iqLeaderThon.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        if query.startswith("(صيانه|صيانه)") and iqLeaderThon.query.user_id == bot.uid:
            try:
                buttons = [[Button.inline("اوامر السورس", data="order1"), Button.inline("اوامر الحساب", data="ord1hs"),],[Button.inline("اوامر الكروب", data="ord1G"), Button.inline("اوامر الالعاب", data="ord1pl"),],[Button.inline("اوامر الصيغ", data="ordsag1"), Button.inline("اوامر الاغاني", data="ordSONG"),], [Button.inline("اسم وقتي", data="order13"), Button.inline("اوامر الاعلانات", data="ordahln1"),],[Button.inline("اوامر التسليه", data="order14"),],[Button.inline("الفارات", data="ordvars"),]]
                result = builder.article(title="iqLeaderThon",text=help2,buttons=buttons,link_preview=False)
                await iqLeaderThon.answer([result] if result else None)
            except BotInlineDisabledError: 
                await iqLeaderThon.send_message( "يجب تفعيل الاونلاين من بوت فاذر اولا " )
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("اوامر الحساب(?: |$)(.*)") and iqLeaderThon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الحساب", data="ord1hs"),]]
            result = builder.article(title="iqLeaderThon", text=help2, buttons=buttons, link_preview=False)
            await iqLeaderThon.answer([result] if result else None)
# GET MY WHISPER
@iqLeaderThon.on(events.NewMessage(pattern=".كشف همسه"))
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("اوامر الالعاب(?: |$)(.*)") and iqLeaderThon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الالعاب", data="ord1pl"),]]
            result = builder.article(title="iqLeaderThon", text=help2, buttons=buttons, link_preview=False)
            await iqLeaderThon.answer([result] if result else None)

chat = "@BotFather"
@iqLeaderThon.on(events.NewMessage(outgoing=True, pattern="^.بوت ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqLeaderThon(iqLeaderThon):
        ...
        query = iqLeaderThon.text
        await bot.get_me()
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and iqLeaderThon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الصيغ", data="ordsag1"),]]
            result = builder.article(title="iqLeaderThon", text=help2, buttons=buttons, link_preview=False)
            await iqLeaderThon.answer([result] if result else None)
@iqLeaderThon.on(admin_cmd(pattern="م21(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 1   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص1`)   ⦙   ابو  عباس  لو  تاكل  خره\n(`.ص2`)   ⦙   استمر  نحن  معك\n(`.ص3`)   ⦙   افحط  بوجه\n(`.ص4`)   ⦙   اكعد  لا  اسطرك  سطره  العباس\n(`.ص5`)   ⦙   اللهم  لا  شماته\n(`.ص6`)   ⦙   امرع  دينه\n(`.ص7`)   ⦙   امشي  بربوك\n(`.ص8`)   ⦙   انت  اسكت  انت  اسكت\n(`.ص9`)   ⦙   انت  سايق  زربه\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص10`)   ⦙   اوني  تشان\n(`.ص11`)   ⦙   برافو  عليك  استادي \n(`.ص12`)   ⦙   بلوك  محترم\n(`.ص13`)   ⦙   بووم  في  منتصف  الجبهة \n(`.ص14`)   ⦙   بيتش \n(`.ص15`)   ⦙   تخوني  ؟\n(`.ص16`)   ⦙   تره  متكدرلي\n(`.ص17`)   ⦙   تعبان  اوي\n(`.ص18`)   ⦙   تكذب\n(`.ص19`)   ⦙   حسبي  الله\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص20`)   ⦙   حشاش \n(`.ص21`)   ⦙   حقير  \n(`.ص22`)   ⦙   خاص  \n(`.ص23`)   ⦙   خاله  ما  تنامون  \n(`.ص24`)   ⦙   خرب  شرفي  اذا  ابقى  بالعراق \n(`.ص25`)   ⦙   دكات  الوكت  الاغبر  \n(`.ص26`)   ⦙   ررردح  \n(`.ص27`)   ⦙   سلامن  عليكم  \n(`.ص28`)   ⦙   بوم منتصف جبهه   \n(`.ص29`)   ⦙   شكد  شفت  ناس  مدودة\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@iqLeaderThon.on(admin_cmd(pattern="م22(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 2   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص30`)   ⦙  شلون  ، \n(`.ص31`)   ⦙  صح  لنوم  \n(`.ص32`)   ⦙  صمت  \n(`.ص33`)   ⦙  ضحكة  مصطفى  الحجي  \n(`.ص34`)   ⦙  طماطه  \n(`.ص35`)   ⦙  طيح  الله  حضك  \n(`.ص36`)   ⦙  فاك  يوو  \n(`.ص37`)   ⦙  اني فرحان وعمامي فرحانين\n(`.ص38`)   ⦙  لا  تضل  تضرط  \n(`.ص39`)   ⦙  لا  تقتل  المتعه  يا  مسلم  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص40`)   ⦙  لا  مستحيل  \n(`.ص41`)   ⦙  لا  والله  شو  عصبي  \n(`.ص42`)   ⦙  لش  \n(`.ص43`)   ⦙  لك  اني  شعليه  \n(`.ص44`)   ⦙  ما  اشرب  \n(`.ص45`)   ⦙  مع  الاسف  \n(`.ص46`)   ⦙  مقتدى  \n(`.ص47`)   ⦙  من  رخصتكم  \n(`.ص48`)   ⦙  منو  انت  \n(`.ص49`)   ⦙  منورني  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص50`)  ⦙  نتلاكه  بالدور  الثاني \n(`.ص51`)  ⦙  نستودعكم  الله  \n(`.ص52`)  ⦙  ها  شنهي  \n(`.ص53`)  ⦙  ههاي  الافكار  حطها ب\n(`.ص54`)  ⦙  ليش شنو سببها ليش\n(`.ص55`)  ⦙  يموتون  جهالي\n(`.ص56`)  ⦙  اريد انام\n(`.ص57`)  ⦙  افتحك فتح\n(`.ص58`)  ⦙  اكل خره لدوخني\n(`.ص59`)  ⦙  السيد شنهو السيد\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص60`)  ⦙  زيج2\n(`.ص61`)  ⦙  زيج لهارون\n(`.ص62`)  ⦙  زيج الناصرية\n(`.ص63`)  ⦙  راقبو اطفالكم\n(`.ص64`)  ⦙  راح اموتن\n(`.ص65`)  ⦙  ذس اس مضرطة\n(`.ص66`)  ⦙  دروح سرسح منا\n(`.ص67`)  ⦙  خويه ما دكوم بيه\n(`.ص68`)  ⦙  خلصت تمسلت ديلة كافي انجب\n(`.ص69`)  ⦙  بعدك تخاف\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@iqLeaderThon.on(admin_cmd(pattern="م23(?: |$)(.*)"))    
async def iq(event):
