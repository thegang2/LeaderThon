...
from . import AUTONAME, BOTLOG, BOTLOG_CHATID, DEFAULT_BIO, _catutils, edit_delete, iqLeaderThon, logging, spamwatch    
...
iqLeaderThonfont = gvarstatus("DEFAULT_PIC") or "Arab/sql_helper/iqLeaderThonIMOGE.ttf"
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        fnt = ImageFont.truetype(f"{iqLeaderThonfont}", 200)
        drawn_text.text((300, 400), current_time, font=fnt, fill=(255, 255, 255))
        img.save(autophoto_path)
        file = await iqLeaderThon.upload_file(autophoto_path)
        try:
            if i > 0:
                await iqLeaderThon(
                    functions.photos.DeletePhotosRequest(
                        await iqLeaderThon.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await iqLeaderThon(functions.photos.UploadProfilePhotoRequest(file))
...
async def crop(imagefile, endname, x):
    ...
    inverted_image.save(endname)
@iqLeaderThon.on(admin_cmd(pattern="احصائيات حسابي(?: |$)(.*)"))
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص1$"))
async def iqvois(vois):
    ...
    if iqvois1:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص2$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois2:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص3$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois3:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="اطار ?(.*)"))
async def memes(mafia):
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص4$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois4:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="قلب الصوره$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="فلتر رصاصي$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@iqLeaderThon.on(events.NewMessage(outgoing=True, pattern='.فك المحظورين'))
async def UnBlockList(event):

    # GET BLOCKED USERS LIST
    list = await iqLeaderThon(GetBlockedRequest(offset=0, limit=1000000))
    ...
    if len(list.blocked) == 0 :
        ...
        for user in list.blocked :
            ...
            # UNBLOCK > USER OR BOT
            UnBlock = await iqLeaderThon(UnblockRequest(id=int(user.peer_id.user_id)))
            unblocked_count += 1
...
iqklanr = ['yes']
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="زوم ?(.*)"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص5$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois5:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص6$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois6:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص7$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois7:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص8$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص9$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص10$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois10:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص11$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois11 :
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص12$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois12:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص13$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois13:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص14$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois14:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص15$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois15:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص16$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois16:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص17$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois17:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص18$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois18:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص19$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois19:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص20$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois20:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص21$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois21:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص22$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois22:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص23$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois23:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص24$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois24:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص25$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois25:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص26$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois26:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص27$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois27:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص28$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois28:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص29$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois29:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص30$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois30:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص31$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois31:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص32$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois32:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص33$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois33:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص34$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois34:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص35$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois35:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص36$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois36:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص37$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois37:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص38$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois38:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص39$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois39:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص40$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois40:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص41$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
@iqLeaderThon.on(admin_cmd(pattern="(تجميع النقاط|تجميع نقاط)"))
async def _(event):
    if iqklanr[0] == "yes":
        ...
        channel_entity = await iqLeaderThon.get_entity(milerbot)
        await iqLeaderThon.send_message(f'{Botcompilation}', '/start')
        await asyncio.sleep(5)
        msg0 = await iqLeaderThon.get_messages(f'{Botcompilation}', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await iqLeaderThon.get_messages(f'{Botcompilation}', limit=1)
        ...
        for i in range(100):
            ...
            list = await iqLeaderThon(GetHistoryRequest(peer=channel_entity, limit=1,
                                                   offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, hash=0))
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await iqLeaderThon.send_message(event.chat_id, f"**لاتوجد قنوات للبوت**")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
                    await iqLeaderThon(JoinChannelRequest(url))
                except:
                    bott = url.split('/')[-1]
                    await iqLeaderThon(ImportChatInviteRequest(bott))
                msg2 = await iqLeaderThon.get_messages(f'{Botcompilation}', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
                await iqLeaderThon.send_message("me", f"تم الاشتراك في {chs} قناة")
            except:
                await iqLeaderThon.send_message(event.chat_id, f"**خطأ حاول بعد 6 ساعات**")
                break
        await iqLeaderThon.send_message(event.chat_id, "**تم الانتهاء من التجميع !**")


@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص42$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois42:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص43$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois43:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص44$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois44:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص45$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois45:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص46$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois46:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص47$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois47:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص48$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois48:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص49$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois49:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص50$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois50:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(pattern="قائمه (جميع القنوات|قنوات اديرها|قنوات امتلكها)$"))
async def stats(event):  
    ...
    try:
        ...
        await edit_or_reply(            catevent,
            output,
            caption=caption        )
@iqLeaderThon.on(admin_cmd(pattern="قائمه (جميع المجموعات|مجموعات اديرها|مجموعات امتلكها)$"))
async def stats(event):  
    catcmd = event.pattern_match.group(1)
    catevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
    ...
    try:
        ...
        await edit_or_reply(
            ...
            output,
            caption=caption        )
@iqLeaderThon.iq_cmd(pattern="حفض كتابه$")
async def saf(e):
...
@iqLeaderThon.iq_cmd(pattern="حفض توجيه$")
async def saf(e):
    x = await e.get_reply_message()
    if not x:
        return await eod(            e, "قم بالرد على أي رسالة لحفظها في رسائلك المحفوظة", time=5        )
    ...
    await eor(e, "تم حفظ الرسالة.", time=5)
@iqLeaderThon.on(admin_cmd(pattern="(الايدي|id)(?: |$)(.*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="وضع بايو(?: |$)(.*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="وضع اسم(?: |$)(.*)"))
async def _(event):
    ...
    try:
        ...
        await edit_or_reply(event, f"**🝳 ⦙  خطأ  ⚠️ :**\n`{str(e)}`")
@iqLeaderThon.on(admin_cmd(pattern="وضع صوره(?: |$)(.*)"))
async def _(event):
    ...
    try:
        ...
        if photo:
            await catevent.edit("**🝳 ⦙   أشترك @iqLeaderThon **")
...
@iqLeaderThon.on(admin_cmd(pattern="وضع معرف(?: |$)(.*)"))
async def update_username(username):
    ...
    try:
        ...
        await edit_or_reply(event, f"**🝳 ⦙  خطأ  ⚠️ :**\n`{str(e)}`")
@iqLeaderThon.on(admin_cmd(pattern=r"شوت ?(.*)", outgoing=True))
...
if 1 == 1:
    ...
    @iqLeaderThon.on(admin_cmd(pattern="الصور ?(.*)"))
    async def potocmd(event):
        ...
        if id.strip() == "":
            ...
            if int(id) <= (len(photos)):
                ...
                return
@iqLeaderThon.on(admin_cmd(pattern="معرفاتي(?: |$)(.*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="ملكيه ([\s\S]*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern=f"{plagiarism}(?: |$)(.*)"))
async def _(event):
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await iqLeaderThon(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus(f"{OR_AUTOBIO}") == "true"
@iqLeaderThon.on(admin_cmd(pattern=f"{unplagiarism}(?: |$)(.*)"))
async def _(event):
    name = f"{DEFAULTUSER}"
    blank = ""
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await iqLeaderThon(functions.account.UpdateProfileRequest(first_name=name))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTONAMESTART = gvarstatus(f"{OR_NAMEAUTO}") == "true"
@iqLeaderThon.on(admin_cmd(pattern="كشف(?:\s|$)([\s\S]*)"))
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if not replied_user:
        return
...
@iqLeaderThon.on(admin_cmd(pattern=f"{idee}(?:\s|$)([\s\S]*)"))
async def who(event):
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
...
@iqLeaderThon.on(admin_cmd(pattern="رابطه(?:\s|$)([\s\S]*)"))
...
@iqLeaderThon.on(admin_cmd(pattern="محادثة وقتية"))
async def _(event):
    ison = getGrChAuto()
...
@iqLeaderThon.on(admin_cmd(pattern="اسمه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@iqLeaderThon.on(admin_cmd(pattern="صورته(?:\s|$)([\s\S]*)"))
async def potocmd(event):
    ...
    if uid.strip() == "":
        ...
        await event.client.send_file(event.chat_id, send_photos)
    await event.delete()  
@iqLeaderThon.on(admin_cmd(pattern=f"{OR_FOTOAUTO}(?: |$)(.*)"))
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@iqLeaderThon.on(admin_cmd(pattern="الملفات ?(.*)"))
async def _(e):
    ...
    try:
        ...
        await e.delete()
@iqLeaderThon.on(admin_cmd(pattern="كول (.*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="كتابه ?(.*)"))
async def writer(e):
    if e.reply_to:
    ...
    for line in lines:
        ...
        y = y + line_height - 5
    file = "iqLeaderThon_Write.jpg"
    img.save(file)
    await e.reply(file=file)
    os.remove(file)
    await e.delete()
@iqLeaderThon.on(admin_cmd(pattern="عد الردود ?(.*)"))
async def _(event):
...
@iqLeaderThon.on(admin_cmd(pattern="زاجل ?(.*)"))
...
@iqLeaderThon.on(admin_cmd(pattern=f"{OR_NAMEAUTO}(?: |$)(.*)"))
async def _(event):
    ...
    await autoname_loop()
@iqLeaderThon.on(admin_cmd(pattern=f"{OR_AUTOBIO}(?: |$)(.*)"))
async def _(event):
    ...
    await autobio_loop()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص51$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois51:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص52$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois52:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص55$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois55:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص54$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois54:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص56$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois56:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص53$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois53:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص57$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois57:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص58$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois58:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص59$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص60$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois60:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص61$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois61:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص62$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois62:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص63$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois63:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص64$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois64:
        ...
        await vois.delete()
@iqLeaderThon.on(events.NewMessage(outgoing=True, pattern="^.ارسل ?(.*)"))
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص65$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois65:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص66$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois66:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص67$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
async def GrChiq_loop():
    ...
    while AUTONAMESTAR:
        ...
        name = f"{AUTOGRCH} {HM}"
        try:
            await iqLeaderThon(functions.channels.EditTitleRequest(channel=await iqLeaderThon.get_entity(int(ag)), title=name))
        except ChatAdminRequiredError:
            await iqLeaderThon.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص68$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois68:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص69$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois69:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص70$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
@iqLeaderThon.iq_cmd(pattern="اشتراك")
...
@iqLeaderThon.iq_cmd(pattern="تعطيل")
async def reda (event):
...
@iqLeaderThon.iq_cmd(incoming=True)
async def reda(event):
    if gvarstatus ("subprivate"):
        if event.is_private:
            try:
                ...
                ch = gvarstatus ("pchan")
                if not ch:
                    return await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                try:
                    ch = int(ch)
                except BaseException as r:
                    return await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                req = requests.get(url)
                reqt = req.text
                if "chat not found" in reqt:
                    mb = await iqLeaderThon .tgbot.get_me()
                    mb = mb.username
                    await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                    return
                if "bot was kicked" in reqt:
                    mb = await iqLeaderThon .tgbot.get_me()
                    mb = mb.username
                    await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                    return
                if "not found" in reqt:
                    try:
                        c = await iqLeaderThon .get_entity(ch)
                        chn = c.username
                        if c.username == None:
                            ra = await iqLeaderThon .tgbot(ExportChatInviteRequest(ch))
                            chn = ra.link
                        ...
                        await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                if "left" in reqt:
                    try:
                        c = await iqLeaderThon .get_entity(ch)
                        chn = c.username
                ...
                if "error_code" in reqt:
                    await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل @lll5l لحل المشكلة\n{reqt}**")
                
                return
            except BaseException as er:
                await iqLeaderThon .tgbot.send_message(BOTLOG_CHATID, f"** حدث خطا\n{er}**")

@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص71$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois71:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص72$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois72:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص73$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois73:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص74$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois74:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص75$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois75:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص76$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois76:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص77$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص78$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois78:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص79$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois79:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص80$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois80:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص81$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois81:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(pattern="عكس الالوان$", outgoing=True))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة...")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص82$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois82:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص83$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois83:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص84$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois84:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص85$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois85:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص86$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois86:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص87$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois87:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص88$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois88:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص89$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois89:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص90$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois90:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص91$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois92:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص92$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois92:
        ...
        await vois.delete()
iqLeaderThonyouali = False
@iqLeaderThon.iq_cmd(pattern="تشغيل حفض الوقتية$")
async def iqalistart(event):
    global iqLeaderThonyouali
    iqLeaderThonyouali = True
    await edit_or_reply(event, "تم بنجاح تفعيل حفظ  الذاتية من الان")
@iqLeaderThon.iq_cmd(pattern="ايقاف حفض الوقتية$")
async def iqalistop(event):
    global iqLeaderThonyouali
    iqLeaderThonyouali = False
    await edit_or_reply(event, "تم بنجاح تعطيل حفظ  الذاتية من الان")
@iqLeaderThon.on(    events.NewMessage(        func=lambda e: e.is_private and (e.photo or e.video) and e.media_unread    ))
async def iqali(event):
    global iqLeaderThonyouali
    if iqLeaderThonyouali:
        sender = await event.get_sender()
        username = sender.username
        user_id = sender.id
        result = await event.download_media()
        caption = (            f" ذاتية التدمير وصلت لك !\n: المرسل @{username}\nالايدي : {user_id}"        )
        await iqLeaderThon.send_file("me", result, caption=caption)
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص93$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois93:
        await vois.client.send_file(vois.chat_id, iqvois93 , reply_to=Ti)
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص94$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois94:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="ص95$"))
async def iqvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if iqvois95:
        ...
        await vois.delete()
@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="فلتر احمر$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة ...")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@iqLeaderThon.iq_cmd(pattern="كروباتي$")
async def gros(event):
    result = await iqLeaderThon(functions.channels.GetGroupsForDiscussionRequest())
    listiq = []
    for iqrusiq in result.chats:
        ...
        listiq.append(kno)
    if listiq:
        await iqLeaderThon.send_message("me", "\n".join(listiq))
@iqLeaderThon.iq_cmd(pattern="الحاظرهم$")
async def bans(event):
    result = await iqLeaderThon(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    listiq = []
    ...
    if listiq:
        await iqLeaderThon.send_message("me", "\n".join(listiq))
@iqLeaderThon.iq_cmd(pattern="قيد (.*)")
async def kade(event):
    exe = event.text[5:]
    try:
        result = await iqLeaderThon(            functions.messages.ToggleNoForwardsRequest(peer=exe, enabled=True)        )
        await event.edit("تم بنجاح تفعيل وضع تقييد المحتوى")
    except errors.ChatNotModifiedError as e:
        print(e)  
@iqLeaderThon.iq_cmd(pattern="نوعه (.*)")
async def noah(event):
    exe = event.text[5:]
    x = await iqLeaderThon.get_entity(exe)
...
@iqLeaderThon.iq_cmd(pattern="احذف (.*)")
async def delet(event):
    exe = event.text[5:]
    await iqLeaderThon.get_dialogs()
    chat = exe
    await iqLeaderThon.delete_dialog(chat, revoke=True)
    await event.edit("- تم بنجاح حذف الدردشة مع المستخدم بنجاح")

@iqLeaderThon.on(admin_cmd(outgoing=True, pattern="يمين الصوره$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
...
@iqLeaderThon.on(admin_cmd(pattern="ايقاف ([\s\S]*)"))
...
iqLeaderThon.loop.create_task(digitalpicloop())
iqLeaderThon.loop.create_task(autoname_loop())
iqLeaderThon.loop.create_task(autobio_loop())
iqLeaderThon.loop.create_task(GrChiq_loop())        
