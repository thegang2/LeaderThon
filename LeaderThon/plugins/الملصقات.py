...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"iqLeaderThonbot_{userid}_{pack}_anim"
    return f"iqLeaderThonbot_{userid}_{pack}"
...
async def add_to_pack(    catevent,    conv,    args,    packname,    pack,    userid,    username,    is_anim,    stfile,    emoji,    cmd,    pkang=False ):
    ...
    return pack, packname
@iqLeaderThon.on(admin_cmd(pattern="صنع ملصق(?:\s|$)([\s\S]*)"))    
async def kang(args):  
    photo = None
    emojibypass = False
    is_anim = False
    ...
    if not user.username:
        try:
            ...
            username = f"iqLeaderThon_{user.id}"
    else:
...
@iqLeaderThon.on(admin_cmd(pattern="جلب الملصقات(?:\s|$)([\s\S]*)"))    
...
@iqLeaderThon.on(admin_cmd(pattern="انشاء حزمه ملصقات(?:\s|$)([\s\S]*)"))   
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "iqLeaderThonbot_" + "".join(        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)    )
...
@iqLeaderThon.on(admin_cmd(pattern="جلب معلومات الملصق$"))    
...
@iqLeaderThon.on(admin_cmd(pattern="ملصق ?([\s\S]*)"))    
