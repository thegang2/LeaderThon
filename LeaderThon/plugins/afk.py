...
AFK_ = AFK()
@iqLeaderThon.iq_cmd(outgoing=True, edited=False)
...
@iqLeaderThon.iq_cmd(    incoming=True, func=lambda e: bool(e.mentioned or e.is_private), edited=False)
...
@iqLeaderThon.iq_cmd(    pattern=f"{sleeping}(?:\s|$)([\s\S]*)",)
async def _(event):
    AFK_.USERAFK_ON = {}
    AFK_.afk_time = None
