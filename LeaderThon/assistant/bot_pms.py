...
from Arab import Config, iqLeaderThon
...
async def bot_start(event):
    ...
    user = await iqLeaderThon.get_me()
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**⌔︙ اهلا وسهلا اخوي  {mention} **\
                        ...
                        \n\n**⌔︙ البوت خاص بسورس :** [TELETHON FOR ARABS](t.me/iqLeaderThon)"
        buttons = [
            (
                Button.url("تنصيب سورسنا", "t.me/iqLeaderThon"),
                Button.url(
                    "قناه المساعد",
                    "https://t.me/LeaderThoniq",
                ),
            )
        ]
    else:
...
@iqLeaderThon.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@iqLeaderThon.bot_cmd(edited=True)
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await iqLeaderThon.tgbot.send_message(
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await iqLeaderThon.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await iqLeaderThon.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await iqLeaderThon.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await iqLeaderThon.tgbot.get_entity(BOTLOG_CHATID)
            await iqLeaderThon.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await iqLeaderThon.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await iqLeaderThon.get_entity(user_id)
    except Exception as e:
...
@iqLeaderThon.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@iqLeaderThon.bot_cmd(incoming=True, func=lambda e: e.is_private)
@iqLeaderThon.bot_cmd(edited=True, func=lambda e: e.is_private)
